...
const TravelModeDropdown = forwardRef<
  TravelModeDropdownHandle,
  TravelModeDropdownProps
>(({ travelMode, setTravelMode }, ref) => {
  ...
  const travelOptions = [
    {
      ...
      label: "Driving",
      animationData: car,
      mainSize: { height: "60px", width: "60px" },
      dropdownSize: { height: "30px", width: "30px" },
    },
    {
      mode: "walking",
      label: "Walking",
      animationData: walking,
      mainSize: { height: "50px", width: "50px", marginLeft: "10px" },
      dropdownSize: { height: "25px", width: "25px" },
    },
    {
      mode: "bicycling",
      label: "Bicycling",
      animationData: bicycle,
      mainSize: { height: "70px", width: "70px" },
      dropdownSize: { height: "35px", width: "35px" },
    },
  ];

  const currentOption = travelOptions.find(
    (option) => option.mode === travelMode,
  );
  const otherOptions = travelOptions.filter(
    (option) => option.mode !== travelMode,
  );
  ...
  return (
    <div className="relative flex-shrink-0" ref={dropdownRef}>
      ...
      {dropdownOpen && (
        <div
          className="absolute left-0 top-12 z-10 rounded border border-gray-200 bg-white shadow-lg transition-opacity duration-300 ease-in-out"
